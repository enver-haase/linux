...
/*
 * Sound-card MMIO registers (lunatix VM). They sit just BELOW the CR block and,
 * like the CRs, are named by an operand whose effective WORD index is negative
 * (src/vm.c is_audio_reg, words -27..-31); as .word operands they are BYTE
 * addresses = word_index * 4. Supervisor-only, WRITE-only (the VM takes the
